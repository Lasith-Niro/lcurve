...
% residual norm || A x - b ||_2 and the solution's (semi)norm || x ||_2
% or || L x ||_2 for a sequence of regularized solutions, ordered such
...
%
% If fig > 0 then a figure with number fig will show the discrete
...
% The philosophy is: select the corner as the rightmost corner
% candidate in the sorted list for which going to the next corner
% candidate yields a larger increase in solution (semi)norm than
% decrease in residual norm, provided that the L-curve is convex
% in the given point. If this is never the case, then select the
% leftmost corner candidate in clist.
...
k_corner = kept(index); % Corner according to original vectors
                        % without Inf, NaN, and zeros removed.
